...
pnpm run build:skill    # Regenerate skills/george-showroom/SKILL.md from shared CLI guidance
...
The committed `skills/george-showroom/SKILL.md` and temporary `skills/lavish/SKILL.md` compatibility alias are generated by `pnpm run build:skill`; `pnpm run check` fails if either drifts from the shared no-args home output.
...
- The product, package, plugin, public skill, and primary CLI are **George Showroom** / `george-showroom`.
- Preserve `lavish-axi` as a temporary CLI alias during the compatibility migration.
- Do not rename inherited `LAVISH_AXI_*` environment variables, `~/.lavish-axi` state, SDK/data attributes, or internal symbols without a separately tested migration.
- `UPSTREAM.md` owns Lavish AXI provenance, the starting commit, MIT obligations, and the upstream sync policy.
- Keep the upstream `LICENSE` copyright and permission text intact.
...
- Runtime guidance strings (`src/cli.js`, `src/design-reference.js`, `src/playbooks.js`) own everything agents are told while using George Showroom; the generated skill mirrors them via `src/skill.js`.
...
George Showroom is a CLI + local HTTP server, derived from Lavish Editor, that opens agent-generated HTML artifacts in a browser, lets the user annotate elements and selected text ranges or edit Mermaid diagrams as whiteboards, and ships that feedback back to the agent over a long-polling API.

### Process model

The primary CLI (`bin/george-showroom.js` -> `src/cli.js`) spawns `george-showroom server` as a **detached** background process (`src/cli.js:startServer`) and waits for `/health`, which returns `{ ok, app, version }`; `bin/lavish-axi.js` remains the compatibility entrypoint.
...
Server self-shutdown keys off live connections (browser SSE and agent polls), not session status, so the next `george-showroom <file>` re-spawns a fresh server and adopts the session from `state.json` when it is still resumable; README's Server cleanup bullet owns the user-facing idle-timeout rules.
...
1. `george-showroom <file.html>` (`openCommand`) -> POST `/api/sessions` -> `SessionStore.upsertSession` -> the CLI opens the returned session URL in the browser.
   `--no-gate` sends `noGate`, returns a one-open URL with `?no-gate=1`, and leaves the stored canonical session URL unchanged.
   Sessions record who ended them: `ended_by: "user"` for browser-initiated ends (including the composer's send-and-end, which posts final prompts and the `endSession` flag to `/api/:key/prompts` atomically) and `ended_by: "agent"` for `george-showroom end <file>`.
...
2. `GET /session/:key` serves a chrome page (`createChromeHtml`) whose sandboxed iframe points at `/artifact/:key/index.html`; the chrome client bootstraps from the `lavish-session` JSON script in the page.
   `extractArtifactHead` (`src/server.js`) adopts the artifact's own `<title>` and `<link rel="icon">` for the chrome tab, but only `data:`, `http(s):`, and protocol-relative icon hrefs - artifact-relative hrefs would not resolve against the chrome page - with fallback to the George Showroom defaults.
...
5. The injected SDK runs the render-time layout audit and posts each pass - findings plus its own `complete` flag and viewport width - to the chrome, which POSTs it to `/api/:key/layout-diagnostics`.
   Detection is **passive**: that route updates the warning inbox and emits an SSE `layout-warnings` event, and it deliberately never emits `feedback`, so a diagnostic pass can never make `george-showroom poll` return or wake an agent. The gate reveals on the first completed pass whatever it found (detection and user-facing behavior: README's Layout issues inbox, Layout issue lifecycle, and Open-time layout gate bullets).
...
7. `george-showroom poll <file.html>` (`pollCommand`) hits `GET /api/poll`.
...
Run `george-showroom server --verbose` (or set `LAVISH_AXI_DEBUG=1`) to log session and watcher events to stderr when diagnosing wedges; detached server stdout/stderr is also appended to `server.log` in `LAVISH_AXI_STATE_DIR` for startup and crash diagnostics.
...
The chrome's **Export standalone HTML** overflow-menu item `GET`s `/api/:key/export`; the CLI exposes the same transform as `george-showroom export`, server-independently.
George Showroom itself sets **no** `Content-Security-Policy` on any response (the sandboxed iframe relies on the `sandbox` attribute, not CSP), but author-set CSP meta tags are preserved and reported as export notices because they may still block exported inline assets.
...
The `home()` callback returns the rich object shown when the user runs `george-showroom` with no arguments - the same TOON-serialized output that lands in the agent's optional `SessionStart` hook after `george-showroom setup hooks`; top-level `--help` returns the same static guidance without dynamic sessions.
...
Agent-facing design guidance is single-sourced in `src/design-reference.js`: `DESIGN_PRIORITY_RULE` states the design-direction priority order exactly once, and `DESIGN_SYSTEM_HINT`, the `george-showroom design` summary, and the design command help all embed it - never restate the rule on another surface (README carries only a one-sentence summary).
`src/skill.js` renders the installable Agent Skill from the same home output, using the trusted locally installed `george-showroom` binary (or its direct global-package path in restricted subprocess sandboxes), omitting live session state, and including Hermes Agent frontmatter metadata for categorization. Until README records an owner-approved npm release, generated skills must prohibit executing `npx -y george-showroom` so the unclaimed registry name is never treated as trusted code.
The generated skill intentionally omits a `version` frontmatter field because release-please updates `package.json` without regenerating `skills/george-showroom/SKILL.md`.
`skills/george-showroom/SKILL.md` is the primary public Agent Skill shipped through npm; `skills/lavish/SKILL.md` is a temporary compatibility alias.
...
The bare-arg form (`george-showroom some.html`) is normalized into `["open", "some.html"]` by `normalizeArgv`, which must let the SDK's `RESERVED_COMMANDS` (such as the built-in `update` self-updater) pass through untouched; otherwise the bare-arg rewrite turns `george-showroom update` into `["open", "update"]` and the inherited reserved command never reaches `runAxiCli`.

### Agent Plugins packaging

The published package root **is** the Agent Plugin: `plugin.json` sits beside the `skills/` directory the standard discovers, so an installed copy is a conformant plugin with nothing downloaded and no marketplace involved (README's Agent Plugin bullet owns the user-facing contract). Keeping that true is the invariant - `plugin.json`, `skills/george-showroom`, and the temporary `skills/lavish` alias must stay in `package.json` `files`, and `resolvePluginRoot` resolves `../` from `src/plugin.js`, which is the package root when bundled and the repo root from source.
...
Telemetry is fail-closed unless both the host and website ID are explicitly configured; George Showroom must never inherit or hardcode an upstream analytics endpoint. Users can also opt out with `LAVISH_AXI_TELEMETRY=0`.
The client is best-effort and must never affect CLI behavior - all errors are swallowed.
Keep the README's concise fail-closed disclosure in sync with this behavior.
...
- The SDK injected into artifacts lives in `src/artifact-sdk.js` and is wrapped by `createSdkJs`.
  ...
  The `george-showroom design` Mermaid snippet chooses dark or light rendering from the effective artifact page background and re-renders when a page-theme or OS appearance change alters that appearance, so do not hardcode one Mermaid theme.
