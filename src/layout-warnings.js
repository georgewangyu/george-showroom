...
export function layoutWarningPromptPayload(warnings) {
  ...
  const prompt =
    ...
    "A queued layout issue is a repair request, not a resolved issue: George Showroom only marks it resolved after a newer artifact load and a complete diagnostic pass for the same viewport no longer detects it.";
  const target = {
